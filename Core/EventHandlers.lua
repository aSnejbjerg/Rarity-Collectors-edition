...
function R:OnItemFound(itemId, item, suppressAlert)
	...
	if item.untrackafterspecificnumber == item.attempts - item.lastAttempts then
		Rarity:Debug(
			format(
				"Item %s has reached its untrackafterspecificnumber (%d) -> Disabling it",
				tostring(item.name),
				tostring(item.untrackafterspecificnumber)
			)
		)
		item.enabled = false
		self:UpdateInterestingThings()
